...
export default class CategoryUpdate extends React.Component {
    constructor(props) {
        super(props);
        const isCreateRoot = props.match.path === "/admin/post/category/create/root";
        this.state = {
            ...
            formRules: {
                templatePath: [
                    {
                        required: true,
                        message: i18n.t("post.templatePathRequired"),
                        trigger: "blur"
                    }
                ],
                displayName: [
                    ...
                    },
                    {
                        validator: (rule, value, callback) => {
                            if (value[value.length - 1] !== "/") {
                                return callback(new Error(i18n.t("post.shouldEndWithSlash")));
                            }
                            if (!isCreateRoot && value === "/") {
                                return callback(new Error(i18n.t("post.shouldNotBeSlash")));
                            }
                            return callback();
                        },
                        trigger: "blur"
            ...
            fileList: [],
            uploading: false,
            directoryPath: "/upload/post/category/",
            templateOptions: [],
            userOptions: [],
            userGroupOptions: [],
            rolesOptions: []
    ...
    componentWillMount() {
        if (this.state.id) {
            ...
            fetch("/admin/api/post/category/" + this.state.form.parentId)
                .then((response) => {
        ...
        fetch("/admin/api/post/template", {method: "GET"})
            .then((response) => {
    ...
    get(id) {
        fetch("/admin/api/post/category/" + id).then((response) => {
            if (response.parentId) {
                fetch("/admin/api/post/category/" + response.parentId)
                    .then((r) => {
                        this.setState({
    ...
    cancel() {
        this.props.history.push("/admin/post/category/list");
    }
    ...
    submit() {
        this.form.validate((valid) => {
            if (valid) {
                if (this.state.id) {
                    fetch("/admin/api/post/category/" + this.state.id, {
                        method: "PUT",
                        body: JSON.stringify(this.state.form)
                    }).then((response) => {
                        notification({
                            title: "Success",
                            message: i18n.t("post.updateSuccessMessage"),
                            type: "success"
                        });
                        this.props.history.push("/admin/post/category/list");
                    }).catch((err) => {
                    ...
                    fetch("/admin/api/post/category", {
                        method: "POST",
                        body: JSON.stringify(this.state.form)
                    }).then((response) => {
                        notification({
                            title: "Success",
                            message: i18n.t("post.createSuccessMessage"),
                            type: "success"
                        });
                        this.props.history.push("/admin/post/category/list");
                    }).catch((err) => {
                ...
                notification({
                    title: i18n.t("post.errorTitle"),
                    type: "error",
                    message: i18n.t("post.errorSubmit")
                });
                return false;
            }
        });
                ...
                <div className="toolbar">
                    <div className="toolbar-form">
                        <Breadcrumb separator="/">
                            ...
                            <Breadcrumb.Item><Link to="/admin/post/category/list">{i18n.t("post.categoryList")}</Link></Breadcrumb.Item>
                            <Breadcrumb.Item>{this.state.id ? i18n.t("post.updateCategory") : i18n.t("post.createCategory")}</Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                    ...
                    <Card>
                        <Form ref={(form) => {
                            ...
                            {this.state.form.parentId || this.state.isCreateRoot
                                ...
                                : ""}
                            <Form.Item label={i18n.t("post.name")} prop="displayName">
                            ...
                            <Form.Item label={i18n.t("post.imageUrl")} prop="imageURL">
                                <Upload
                            ...
                            <Form.Item label={i18n.t("post.description")}>
                            ...
                            {
                                this.state.form.fields && <Form.Item label={i18n.t("post.fields")}>
