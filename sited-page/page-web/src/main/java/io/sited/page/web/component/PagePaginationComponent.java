...
public class PagePaginationComponent extends TemplateComponent {
    ...
    public void output(Map<String, Object> bindings, Map<String, Object> attributes, Children children, OutputStream out) throws IOException {
        QueryResponse items = (QueryResponse) attribute("items").value(attributes);
        if (items == null) {
            return;
        }

        int display = (Integer) attribute("display").value(attributes);
        if (items.total <= display) {
            return;
        }

        String path = (String) attribute("path").value(attributes);
