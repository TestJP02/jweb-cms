...
public class PageCategoryService {
    ...
    public PageCategory create(CreateCategoryRequest request) {
        ...
        pageCategory.keywords = request.keywords == null ? null : Joiner.on(';').skipNulls().join(request.keywords);
        pageCategory.tags = request.tags == null ? null : Joiner.on(';').skipNulls().join(request.tags);
        pageCategory.status = CategoryStatus.ACTIVE;
        pageCategory.ownerId = request.ownerId;
        pageCategory.ownerRoles = request.ownerRoles == null ? null : Joiner.on(';').skipNulls().join(request.ownerRoles);
        pageCategory.groupId = request.groupId;
        pageCategory.groupRoles = request.groupRoles == null ? null : Joiner.on(';').skipNulls().join(request.groupRoles);
        pageCategory.othersRoles = request.othersRoles == null ? null : Joiner.on(';').skipNulls().join(request.othersRoles);
