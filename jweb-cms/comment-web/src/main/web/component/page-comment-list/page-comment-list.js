...
Comment = {
    ...
    init: function() {
        ...
        this.$container.on("click", ".comment-vote-btn", function(e) {
            this.vote($(e.currentTarget));
        }.bind(this));
        ...
        $.ajax({
            ...
            const visitorCommentEnabled = $(".user-comment-text").attr("visitorCommentEnabled");
            if (!response.authenticated && visitorCommentEnabled === "false") {
                $(".user-comment-text").prop("disabled", true);
                $(".user-comment-submit").hide();
                $(".comment-reply-btn").hide();
                $(".user-comment-login-button").show();
            }
        }).fail(function() {
    ...
    submit: function($e) {
        ...
        $.ajax({
            ...
            if (response.comment.parentId) {
                ...
                var totalRepliesElement = $e.parent().siblings(".comment-btn-group").find(".comment-total-replies");
                var totalRepliesText = totalRepliesElement.html();
                var totalReplies = parseInt(totalRepliesText.substring(1, totalRepliesText.length - 1)) + 1;
                totalRepliesElement.html("(" + totalReplies + ")");
            } else {
    ...
    reply: function($e) {
        var $replyContent = $e.parent().siblings(".reply-content");
    ...
    vote: function($e) {
        var id = $e.data("id");
        $.ajax({
            url: "/web/api/comment/" + id + "/vote",
            method: "POST",
            dataType: "json"
        }).then(function(response) {
            $e.find(".comment-vote-up").html('(' + response.totalVoteUp + ')');
        }.bind(this));
    },
